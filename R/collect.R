...
#' few seconds - anything not fetched is lost forever. \code{rt_collect()}
...
#' @export
rt_collect <- function(config, dir, max_polls = Inf) {
...
#'
#' @param dir Archive root directory (as used by \code{\link{rt_collect}}).
#' @return Invisibly, a character vector of the ZIP files written.
#' @export
rt_rotate <- function(dir) {
  if (!nzchar(Sys.which("zip"))) {
    stop("rt_rotate() requires the 'zip' system utility on the PATH.", call. = FALSE)
  }
...
#' \code{\link{rt_collect}} running unattended: a systemd unit, a launchd
...
rt_service_template <- function(
  ...
  type <- match.arg(type)
  cmd <- sprintf(
    "Rscript -e 'gtfsrt2static::rt_collect(read.csv(\"%s\"), dir = \"%s\")'",
    config_path,
    dir
  )
  tpl <- switch(
