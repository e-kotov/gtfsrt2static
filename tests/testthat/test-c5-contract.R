...
#   2. rt2s_events_from_stop_times maps orientation_id INTO direction_id, falling
...
test_that("C5 columns do not perturb C6 assembly (round-trip)", {
  legacy <- rt2s_events_from_stop_times(make_g2g_stop_times())
  withc5 <- rt2s_events_from_stop_times(make_g2g_stop_times_c5())
  ...
  feed_legacy <- suppressWarnings(rt2s_scaffold(legacy))
  feed_withc5 <- suppressWarnings(rt2s_scaffold(withc5))
...
test_that("detector-off: direction_id still derives from legacy direction", {
  ...
  events <- rt2s_events_from_stop_times(make_g2g_stop_times_c5())
...
test_that("orientation_id, when set, overrides direction_id per row", {
  ...
  ev2 <- rt2s_events_from_stop_times(st2)
  expect_true(all(ev2$direction_id == 0L))
})

test_that("rt2s_events_validate tolerates a present or absent pattern_ref", {
  events <- rt2s_events_from_stop_times(make_g2g_stop_times_c5())
  expect_silent(rt2s_events_validate(events))
  # Non-character pattern_ref is coerced, not rejected.
  events2 <- data.table::copy(events)
  events2[, pattern_ref := 1L]
  expect_silent(v <- rt2s_events_validate(events2))
