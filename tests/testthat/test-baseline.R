...
test_that("baseline_patterns picks the modal signature and rebases offsets", {
  p <- baseline_patterns(make_baseline_freq())
...
test_that("baseline_patterns tie-breaks on stop count, then lexicographically", {
  p <- baseline_patterns(make_baseline_tied_patterns())
...
test_that("baseline_patterns is independent of input row order", {
  ...
  expect_identical(
    as.data.frame(baseline_patterns(shuffled)),
    as.data.frame(baseline_patterns(b))
  )
})

test_that("baseline_patterns excludes trips with missing times, not routes", {
  # T1 loses one arrival, so candidacy falls to T2/T3 and the template moves.
  expect_warning(
    p <- baseline_patterns(make_baseline_partial_times()),
...
test_that("baseline_patterns errors when a route-direction loses every trip", {
  b <- make_baseline_freq()
  b$stop_times$arrival_time <- NA_character_
  expect_error(
    suppressWarnings(baseline_patterns(b)),
    "interpolate"
  )
})

test_that("baseline_patterns requires direction_id rather than guessing it", {
  expect_error(
    baseline_patterns(make_baseline_no_direction()),
    "direction_id"
  )
})

test_that("baseline_patterns warns on and excludes NA direction_id", {
  b <- make_baseline_freq()
  b$trips$direction_id[b$trips$trip_id == "T1"] <- NA_integer_
  expect_warning(
    p <- baseline_patterns(b),
    "NA 'direction_id'"
  )
...
test_that("baseline_patterns honours min_stops", {
  ...
  expect_error(baseline_patterns(make_baseline_freq(), min_stops = 4L), "usable")
  expect_error(baseline_patterns(make_baseline_freq(), min_stops = 1L), ">= 2")
})

test_that("baseline_patterns can key on route_short_name", {
  p <- baseline_patterns(make_baseline_freq(), route_key = "route_short_name")
  expect_identical(unique(p$route_ref), "1")

  b <- make_baseline_freq()
  b$routes <- NULL
  expect_error(
    baseline_patterns(b, route_key = "route_short_name"),
    "routes.txt"
  )
})

test_that("baseline_patterns rejects malformed baseline input", {
  b <- make_baseline_freq()
  b$stop_times <- NULL
  expect_error(baseline_patterns(b), "stop_times.txt")

  b2 <- make_baseline_freq()
  b2$trips$trip_id[2] <- "T1"
  expect_error(baseline_patterns(b2), "duplicate 'trip_id'")

  b3 <- make_baseline_freq()
  b3$stop_times$stop_sequence[1:3] <- 4L
  expect_error(baseline_patterns(b3), "repeated 'stop_sequence'")

  b4 <- make_baseline_freq()
  b4$stop_times$departure_time[1] <- "05:00:00"
  expect_error(baseline_patterns(b4), "depart before they arrive")
})

test_that("baseline_patterns accepts numeric second columns and >24h clocks", {
  b <- make_baseline_freq()
...
test_that("baseline_headways summarises the planned trip-start gaps", {
  h <- baseline_headways(
    make_baseline_freq(),
...
test_that("baseline_headways splits by window and drops single departures", {
  h <- baseline_headways(
    make_baseline_freq(),
...
test_that("baseline_headways excludes gaps above max_headway_secs", {
  h <- baseline_headways(
    make_baseline_freq(),
    windows = list(am_peak = c("06:00", "09:00")),
...
test_that("baseline_headways handles overnight windows past 24:00:00", {
  b <- make_baseline_freq()
  b$stop_times$arrival_time <- sub("^06:", "25:", b$stop_times$arrival_time)
  b$stop_times$departure_time <- sub("^06:", "25:", b$stop_times$departure_time)
  b$stop_times$arrival_time <- sub("^05:59", "24:59", b$stop_times$arrival_time)
  h <- baseline_headways(b, windows = list(overnight = c("24:00", "26:00")))
  expect_identical(h$headway_secs, 750L)
  expect_identical(h$window, "overnight")
})

test_that("baseline_headways errors when nothing falls inside a window", {
  expect_error(
    baseline_headways(
      make_baseline_freq(),
  ...
  expect_error(baseline_headways(make_baseline_freq()), "windows")
})
